#Credit to HaltonCoders for the amazing pong code
...
cpuPaddle = pygame.rect.Rect(winWidth - 2*paddleWidth, (winHeight - paddleHeight)//2, paddleWidth, paddleHeight)
player1score, cpuScore = 0, 0
...
def reset_game():
    global ballSpeedX, ballSpeedY
    global player1Paddle, cpuPaddle
    global player1score, cpuScore
    ball.center = (winWidth / 2, winHeight / 2)
    ballSpeedX *= random.choice((1, -1))
    ballSpeedY *= random.choice((1, -1))
    player1score = 0
...
while True:
    ...
    if cpuPaddle.y > ball.y and cpuPaddle.y >= 10:
        cpuPaddle.y -= cpuPaddleSpeed
    elif cpuPaddle.y < ball.y and cpuPaddle.y <= winHeight - (paddleHeight + paddleSpeed + 2):
        cpuPaddle.y += cpuPaddleSpeed
    ...
    pygame.draw.rect(win, paddleColor, cpuPaddle)
    ...
    if player1score == 5:
        ...
        cpuWinText = scoreFont.render('Cpu Wins!', False, Colors["white"])
        win.blit(cpuWinText, (500, 100))
        resetGameText = scoreFont.render('To play again press "r"', False, Colors["white"])
    ...
    cpuScoreText = scoreFont.render(str(cpuScore), False, Colors["white"])
    win.blit(player1ScoreText, (370, 290))
    win.blit(cpuScoreText, (420, 290))
